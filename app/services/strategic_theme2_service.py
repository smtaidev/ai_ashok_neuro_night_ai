...
async def generate_combined_analysis(request: CombinedAnalysisRequest) -> CombinedResponse:
    ...
    try:
        ...
        # Run the remaining analyses concurrently
        gap_result, wording_result, goal_result = await asyncio.gather(
            generate_gap_detection(gap_request),
            generate_wording_suggestions(wording_request),
            generate_goal_mapping(goal_request)
        )
 
        # Assemble the response without benchmarking
...
# --- Individual Service Functions (Benchmarking function removed) ---
 
async def generate_gap_detection(request: GapDetectionRequest) -> GapDetectionResponse:
    system_prompt = f"""
    You are a strategic AI advisor. Analyze the user's themes and context.
